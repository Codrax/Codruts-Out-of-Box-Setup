...
interface
  ...
  procedure DrawDottedLine(Canvas: TCanvas; Line: TLine; LinWidth: integer = 2);
  ...
  procedure DrawImageInRect(Canvas: TCanvas; Rect: TRect; Image: TGraphic; DrawMode: TDrawMode = dmFill; ImageMargin: integer = 0; ClipImage: boolean = false);
...
procedure DrawDottedLine(Canvas: TCanvas; Line: TLine; LinWidth: integer);
{var
  WLength: Real; }
begin
  {WLength := DistancePoints( Line.Point1.X, Line.Point1.Y, Line.Point2.X, Line.Point2.Y );

  if LinWidth >= WLength then
    LinWidth := round(WLength / 3);}
end;
